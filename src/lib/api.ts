...
export const setMaster = (enabled: boolean) =>
  invoke<AppStatus>("set_master", { enabled });
...
export const getCurve = (deviceGuid: string) =>
  invoke<CurveData>("get_curve", { deviceGuid });
export const setBassShelf = (deviceGuid: string, gainDb: number, fc: number) =>
  invoke<AppStatus>("set_bass_shelf", { deviceGuid, gainDb, fc });
export const setHeadroom = (db: number) =>
  invoke<AppStatus>("set_headroom", { db });
...
export function searchGroups(groups: GroupedHeadphone[], query: string, limit = 30): GroupedHeadphone[] {
